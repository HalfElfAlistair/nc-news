...
const Votes = ({article, setError}) => {
    ...
    return <div id="votes">
        <p>{voteCount} votes</p>
        <button className=".icon-button" aria-label="thumbs up" disabled={disable} onClick={(e) => {
            setVoteCount((currVotes) => updateVote(currVotes, 1))
            setResetValue(-1)
            handleVote(1)
            setDisable(true)
            }}><BsHandThumbsUpFill/></button>
        <button className=".icon-button" aria-label="thumbs down" disabled={disable} onClick={(e) => {
            setVoteCount((currVotes) => updateVote(currVotes, -1))
            setResetValue(1)
            handleVote(-1)
            setDisable(true)
        ...
        {disable === true
         ? 
         <button className=".icon-button" aria-label="reset" onClick={(e) => {
